...
import { fileHashMd5, getFileType } from 'src/utils';
import {
  FileFileDTO,
  FileChunkFileDTO,
  FileExistFileDTO,
...
export class FileController {
  ...
  async delFile(@Body() params: FileDelDTO) {
    ...
    fs.moveSync(path.join(process.cwd(), info.url), path.join(process.cwd(), delPath, `${info.md5}${info.extname}`));
    return { message: '删除成功' };
  }
  ...
  async singleFile(@Req() req, @Body() params: FileParamsDTO, @UploadedFile() file: Express.Multer.File) {
    try {
      const userId = req.user.userId;
      ...
      const url = assetsPath + `/${getFileType(extname)}/${md5}${extname}`;
  ...
  async merge(@Req() req, @Query() params: FileMergeFileDTO) {
    ...
    if (!fs.existsSync(filePath)) {
      try {
        ...
        ws.close();
        fs.removeSync(dir);
        const url = assetsPath + `/${getFileType(extname)}/${hash}${extname}`;
        const info = await this.fileService.create({
          filename: filename,
          url: url,
          md5: hash,
